    ...
    <v-main>
      <v-container class="px-6 mt-n2" >
        <v-row>
          ...
          <v-card class="login__form" >
            <h3 style="color: rgb(22, 22, 22); text-align: center;" @click="step=2" >USERS</h3>
            <div class="search">
             <input type="text"  placeholder=" search for user" v-model="search" >
            <button  @click.prevent="getSearch()" >search</button>
          </div>
            <v-list class="listt"  >
              <!-- <v-list-item  v-for=" v,index in users" :class="{ active: activeIndex === index }">
            <div class="box"  @click.prevent="save(v.id,index)">
			<header class="header">
				<div class="main">
          <i class="icon"></i>
          <div style="margin: 0 10px; "><p style="color: black;">{{ v["user_name"] }}</p>
          <p >{{ v["user_email"] }}</p>
        </div>
				</div>
        <btu v-if="v['user_blocked']== 'True'" class="login__button3" style="text-align: center; background-color: brown;" @click.prevent="unBlockUser(v.id)">Unblocked</btu>
			</header>
		             </div>
                </v-list-item> -->
            ...
            <header class="header">
              <div class="main">
                <i class="icon"></i>
                <div style="margin: 0 10px; "><p style="color: black;">{{ "user_name"}}</p>
                <p >{{ "user_email" }}</p>
              </div>
             <!-- <h4 style="position: absolute; left: 270px;">{{ v["User's Phone"] }}</h4>  -->
              </div>
              <btu v-if="'user_blocked'== 'True'" class="login__button3" style="text-align: center; background-color: brown;" >Unblocked</btu>
            </header>
                       </div>
                      </v-list-item>
                      <v-list-item   :class="{ active: activeIndex === index }">
            ...
            <div class="btu">
             ...
             <button type="submit" tile outlined dark  class="login__button2">Edit</button>
             <button type="submit"  class="login__button2">Upgrade</button>
            </div>
          </v-card>
        </v-col>
      </v-window-item>
      ...
      />
              ...
              <div class="login__box">
               ...
               <br>
               <br>
               <button  @click.prevent="updateUser(ID)" class="login__button">Save</button>
...
this.activeIndex = index;
    ...
    gitID2(){
      if (this.ID == '') {
        ...
        axios
    .request({
      url: "http://127.0.0.1:8000/api/admin/users/"+this.ID+"/block/",
      method: "put",
       headers: {
    ...
    .request({
      ...
      unBlockUser(id) {
        axios
    .request({
      url: "http://127.0.0.1:8000/api/admin/users/"+id+"/unblock/",
      method: "put",
       headers: {
        Authorization: "Bearer " + this.token,
       },
